...
export const Menu = ({ children }) => {
  ...
  const { stateLogin: { usuario, isAuth } } = useContext
  (LoginContext)
          ...
          { sidebar ? (
            <div className='d-flex align-items-center gap-3 pb-3 ps-1 justify-content-around'>
              <div className='d-flex align-items-center gap-2'>
                { usuario && isAuth ? (
                  <>
                    <img width='50px' height='50px' style={{display: sidebar ? "block": "none", borderRadius: '30px'}} className="logo " src={ usuario.imagen ? usuario.imagen : 'https://cdn-icons-png.flaticon.com/512/6073/6073873.png'} alt='Esta es una imagen de un usuario'/>
                    <strong className='text-capitalizept-2'>{usuario.nombre} {usuario.apellido}</strong> 
                  </>
                ):
                (
                  <>
                    <img width='50px' height='50px' style={{display: sidebar ? "block": "none", borderRadius: '30px'}} className="logo " src='https://cdn-icons-png.flaticon.com/512/6073/6073873.png' alt='Esta es una imagen por defecto'/>
          ...
          )}
          {
            menuItems.map((item, index) => (
              <NavLink to={item.path} key={index} className="link">
                ...
                <div style={{display: sidebar ? "block": "none"}} className="link_text">{item.name}</div>
              </NavLink>
            ))
          }
          </div>
